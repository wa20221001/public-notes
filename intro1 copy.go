...
// Problem: Write a function that takes an array of integers and returns a new array with the elements in reverse order.
// Example: Given [1, 2, 3, 4, 5], the function should return [5, 4, 3, 2, 1].
package main
...
func testFirstUniqueChar() {
	tests := []struct {
		...
		{"ab", 0},             // 'a' is the first unique character
		{"aabbccddeeffg", 12}, // 'g' is the first unique character
	}

	for _, test := range tests {
		result := firstUniqueChar(test.input)
		if result != test.expected {
			fmt.Printf("Test failed: firstUniqueChar(%s) = %v; expected %v\n", test.input, result, test.expected)
		} else {
			fmt.Printf("Test passed: firstUniqueChar(%s)\n", test.input)
		}
	}
}
